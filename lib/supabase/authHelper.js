...
function parseJwtPayload(token) {
  try {
    ...
    // Validate expiration (allow 60-second clock skew buffer)
...
async function resolveUserFromToken(adminSupabase, token) {
  ...
  // 2. Attempt GoTrue verification with a short 800ms guard; fall back seamlessly to jwtUser
  try {
    const authPromise = adminSupabase.auth.getUser(token);
    const timeoutPromise = new Promise((_, reject) =>
      setTimeout(() => reject(new Error("GET_USER_TIMEOUT")), 800)
    );

    const { data: { user } = {} } = await Promise.race([authPromise, timeoutPromise]);
    if (user) return user;
  } catch {
    // GoTrue unresponsive or high latency: jwtUser already validated
  }
...
export async function getAuthUser(req, supabaseServer) {
  try {
    ...
    if (cookieHeader) {
      // A. Match direct JWT token anywhere in the cookie header
      const jwtMatch = cookieHeader.match(/eyJ[a-zA-Z0-9_-]+\.eyJ[a-zA-Z0-9_-]+\.[a-zA-Z0-9_-]+/);
      if (jwtMatch && jwtMatch[0]) {
        const directJwtUser = await resolveUserFromToken(adminSupabase, jwtMatch[0]);
      ...
      if (rawVal) {
        ...
        if (jsonStr.startsWith("base64-")) {
          try {
            jsonStr = Buffer.from(jsonStr.slice(7), "base64").toString("utf8");
          } catch (_) {}
        }
        try {
          ...
          // If not valid JSON, test if rawVal itself is a token
          const fallbackUser = await resolveUserFromToken(adminSupabase, rawVal);
