    ...
    const employeeRecords = reportStaff.map((emp) => {
      ...
      if (att) {
        ...
        return {
          ...
          checkIn: formatTime12h(att.check_in, "Asia/Kolkata"),
          checkOut: formatTime12h(att.check_out, "Asia/Kolkata"),
          delayedDuration,
          isLate,
          isEarly,
    ...
    const sendResults = await Promise.allSettled(
      emailList.map((targetEmail) =>
        transporter.sendMail({
          from: senderFrom,
          to: targetEmail,
          subject: `Daily Attendance Summary Report (${dateStr})`,
          html: emailHTML,
        })
      )
    );
