...
/**
 * Extracts the /64 routing prefix (first 4 hextets) from an IPv6 address string.
 */
function getIpv6Prefix(ip) {
  if (!ip || !ip.includes(":")) return null;
  const clean = ip.toLowerCase().split("%")[0].trim();
...
/**
 * Evaluates whether a client IP matches an individual network rule (exact IP, CIDR range, IPv6 prefix, or same /24 private subnet).
 */
export function isIpMatching(clientIp, networkRule) {
  ...
  // 3. Prefix match (e.g., "192.168.1." or "192.168.1.*" or "10.0." or "2406:7400:")
  const prefixPattern = cleanRule.replace(/\*$/, "");
  if (cleanClient.startsWith(prefixPattern)) {
    return true;
  }

  // 4. IPv6 Subnet / Link-Local comparison
  ...
  // 5. CIDR Subnet match (e.g., 192.168.1.0/24 or 10.0.0.0/16)
  if (cleanRule.includes("/") && !cleanClient.includes(":")) {
    try {
  ...
  // 6. Automatic /24 subnet match for private class C IPv4 (e.g., router 192.168.1.1 vs device 192.168.1.44)
...
export async function validateCompanyNetwork(req, companyId, adminSupabase) {
  ...
  try {
    ...
    // 3. Dual-Stack Wi-Fi Resolution for Mobile Devices (IPv6 over Wi-Fi Router):
    // When a mobile phone on the same office Wi-Fi connects over IPv6, its /64 prefix
    // (e.g. 2406:7400:ca:f9a8:*) matches the server's Wi-Fi interface IPv6 prefix.
    // Verify that the server's Wi-Fi interface is on an active company network.
    if (isClientIpv6) {
      const clientPrefix = getIpv6Prefix(clientIp);
      const isClientLinkLocal = clientIp.toLowerCase().startsWith("fe80:");

      // Check if host machine Wi-Fi is on authorized company network
      let hostWifiAuthorizedNet = null;
      for (const localIp of localIps) {
        for (const net of activeList) {
          if (isIpMatching(localIp, net.network_ip)) {
            hostWifiAuthorizedNet = net;
            break;
          }
        }
        if (hostWifiAuthorizedNet) break;
      }

      if (hostWifiAuthorizedNet) {
        // If client is link-local or matches host's Wi-Fi IPv6 /64 prefix
        for (const localIp of localIps) {
          if (localIp.includes(":")) {
            ...
            if ((isClientLinkLocal && localIp.toLowerCase().startsWith("fe80:")) || (clientPrefix && hostPrefix && clientPrefix === hostPrefix)) {
              return {
                isAuthorized: true,
                clientIp,
                matchedNetwork: hostWifiAuthorizedNet,
                activeNetworksCount: activeList.length,
              };
            }
          }
