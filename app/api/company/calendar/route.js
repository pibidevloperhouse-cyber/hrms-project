...
export async function GET(req) {
  try {
    ...
    const { data: holidayRows, error: holErr } = await adminSupabase
      .from("company_holidays")
      .select("*")
      .eq("company_id", company.id)
      .order("date", { ascending: true });

    if (holErr && holErr.code !== "42P01") {
      console.warn("Notice fetching holidays:", holErr.message);
    }
    ...
    const holidays = (holidayRows || []).map((h) => ({
      ...
      title: h.title,
      date: h.date,
      holidayType: h.holiday_type || "Company Holiday",
