    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <div className="p-4 rounded-xl bg-amber-50 border border-amber-200 space-y-1">
          ...
          <span className="text-[10px] text-amber-600">&lt;8h reason submitted</span>
        </div>
                ...
                return (
                  <tr key={emp.employeeId} className={`hover:bg-sky-50/50 transition group ${isPendingHR ? "bg-amber-50/60" : ""}`}>
                    ...
                    <td className="py-3.5 px-4 font-mono">
                      <span className={`font-bold ${emp.status === "COMPANY_HOLIDAY" ? "text-purple-700" : emp.status === "ON_LEAVE" ? "text-cyan-700" : emp.workingHours < 8.0 && emp.status !== "NOT_CHECKED_IN" ? "text-amber-700" : "text-emerald-700"}`}>
                        {emp.status === "NOT_CHECKED_IN" ? "0.00 hrs" : `${emp.workingHours.toFixed(2)} hrs`}
                      </span>
                      {emp.status === "COMPANY_HOLIDAY" && (
                        <span className="block text-[9px] font-sans text-purple-700 font-semibold">Holiday Credit (+8.0h)</span>
                      )}
                      {emp.status === "ON_LEAVE" && (
                        <span className="block text-[9px] font-sans text-cyan-700 font-semibold">Leave Credit (+8.0h)</span>
                      )}
                      {emp.workingHours < 8.0 && emp.status !== "NOT_CHECKED_IN" && emp.status !== "CHECKED_IN" && emp.status !== "ON_LEAVE" && emp.status !== "COMPANY_HOLIDAY" && (
                        <span className="block text-[9px] font-sans text-amber-700">Under 8h standard</span>
                      )}
                    </td>
